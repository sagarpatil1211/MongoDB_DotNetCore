...
{
    public class Tag_Pool_Migration
    {
        private readonly MongoDbContext _mongoDbContext;

        private readonly SQLDatabaseContext _sqlDbContext;
        private readonly IMongoCollection<Tag_Pool> _Tag_Pool;
        public Tag_Pool_Migration(MongoDbContext mongoDbContext, SQLDatabaseContext sqlDbContext)
        {
            _mongoDbContext = mongoDbContext;
            _Tag_Pool = mongoDbContext.GetCollection<Tag_Pool>("Tag_Pool");
            _sqlDbContext = sqlDbContext;
        }
        public async Task addTag_PoolRecentRecords(DateTimeOffset input1)
        {
            ...
            var filter = Builders<Tag_Pool>.Filter.And(
                Builders<Tag_Pool>.Filter.Gte(x => x.enddate, Convert.ToDateTime(strFiveMinutesAgo)), // Greater than or equal to
                Builders<Tag_Pool>.Filter.Lte(x => x.enddate, Convert.ToDateTime(strCurrentDate))    // Less than or equal to
            );

            DateTime? latestCaptuteEndDate;
            FilterDefinition<Tag_Pool> filterByLatestdate = null;
            var latestCaptuteEndDateRecord = await _sqlDbContext.LatestCapturedEndDate.FirstOrDefaultAsync(e => e.collectionName == "Tag_Pool");
            if (latestCaptuteEndDateRecord != null)
            {
                ...
                if (latestCaptuteEndDate != DateTime.MinValue)
                {
                    string strlatestCaptuteEndDate = latestCaptuteEndDate?.ToString("yyyy-MM-ddTHH:mm:ss.fff+00:00");
                     filterByLatestdate = Builders<Tag_Pool>.Filter.And(
                        Builders<Tag_Pool>.Filter.Gt(x => x.enddate, Convert.ToDateTime(strlatestCaptuteEndDate)) // Greater than   
                        );

                }
            }

            FilterDefinition<Tag_Pool> combinedFilter = null;


            if (filterByLatestdate is not null)
            {
                combinedFilter = Builders<Tag_Pool>.Filter.And(
                filterByLatestdate,
                filter 
                );
            }
            ...
            var records = await _Tag_Pool.Find(combinedFilter)
                .Sort(Builders<Tag_Pool>.Sort.Descending(x => x.enddate)) // Sort by enddate descending
                //.Limit(25)
                .ToListAsync();


            var a = new List<Tag_Pool>();
            ...
            using (var transaction = await _sqlDbContext.Database.BeginTransactionAsync())
            {
                try
                {
                    if (records.Count > 0)
                    {
                        ...
                        {
                            var sqlRecord = new Sql_Tag_Pool
                            {
                            ...
                            await _sqlDbContext.Tag_Pool.AddAsync(sqlRecord);
                        }
                ...
                try {
                    if(records.Count > 0) {
                    var existingRecord = await _sqlDbContext.LatestCapturedEndDate.FirstOrDefaultAsync(e => e.collectionName == "Tag_Pool");
