...
{
    public class L1_Pool_Migration
    {
        private readonly MongoDbContext _mongoDbContext;

        private readonly SQLDatabaseContext _sqlDbContext;
        private readonly IMongoCollection<L1_Pool> _L1_Pool;
        public L1_Pool_Migration(MongoDbContext mongoDbContext, SQLDatabaseContext sqlDbContext)
        {
            _mongoDbContext = mongoDbContext;
            _L1_Pool = mongoDbContext.GetCollection<L1_Pool>("L1_Pool");
            _sqlDbContext = sqlDbContext;
        }
        public async Task addL1_PoolRecentRecords(DateTimeOffset input1)
        {
            ...
            var filter = Builders<L1_Pool>.Filter.And(
                Builders<L1_Pool>.Filter.Gte(x => x.enddate, Convert.ToDateTime(strFiveMinutesAgo)), // Greater than or equal to
                Builders<L1_Pool>.Filter.Lte(x => x.enddate, Convert.ToDateTime(strCurrentDate))    // Less than or equal to
            );

            DateTime? latestCaptuteEndDate;
            FilterDefinition<L1_Pool> filterByLatestdate = null;
            var latestCaptuteEndDateRecord = await _sqlDbContext.LatestCapturedEndDate.FirstOrDefaultAsync(e => e.collectionName == "L1_Pool");
            if (latestCaptuteEndDateRecord != null)
            {
                ...
                if (latestCaptuteEndDate != DateTime.MinValue)
                {
                    string strlatestCaptuteEndDate = latestCaptuteEndDate?.ToString("yyyy-MM-ddTHH:mm:ss.fff+00:00");
                     filterByLatestdate = Builders<L1_Pool>.Filter.And(
                        Builders<L1_Pool>.Filter.Gt(x => x.enddate, Convert.ToDateTime(strlatestCaptuteEndDate)) // Greater than   
                        );

                }
            }

            FilterDefinition<L1_Pool> combinedFilter = null;


            if (filterByLatestdate is not null)
            {
                combinedFilter = Builders<L1_Pool>.Filter.And(
                filterByLatestdate,
                filter 
                );
            }
            ...
            var records = await _L1_Pool.Find(combinedFilter)
                .Sort(Builders<L1_Pool>.Sort.Descending(x => x.enddate)) // Sort by enddate descending
                //.Limit(25)
                .ToListAsync();


            var a = new List<L1_Pool>();
            ...
            using (var transaction = await _sqlDbContext.Database.BeginTransactionAsync())
            {
                try
                {
                    if (records.Count > 0)
                    {
                        ...
                        foreach (var record in records)
                        {
                            ...
                            await _sqlDbContext.L1_Pool.AddAsync(sqlRecord);
                        }
                ...
                try {
                    if(records.Count > 0) {
                    var existingRecord = await _sqlDbContext.LatestCapturedEndDate.FirstOrDefaultAsync(e => e.collectionName == "L1_Pool");
