...
{
    public class Operator_History_Transfer
    {
        private readonly MongoDbContext _mongoDbContext;

        private readonly SQLDatabaseContext _sqlDbContext;
        private readonly IMongoCollection<Operator_History> _Operator_History;
        public Operator_History_Transfer(MongoDbContext mongoDbContext, SQLDatabaseContext sqlDbContext)
        {
            _mongoDbContext = mongoDbContext;
            _Operator_History = mongoDbContext.GetCollection<Operator_History>("Operator_History");
            _sqlDbContext = sqlDbContext;
        }
        public async Task addOperator_HistoryRecentRecords(DateTimeOffset input1)
        {
            ...
            var filter = Builders<Operator_History>.Filter.And(
                Builders<Operator_History>.Filter.Gte(x => x.enddate, Convert.ToDateTime(strFiveMinutesAgo)), // Greater than or equal to
                Builders<Operator_History>.Filter.Lte(x => x.enddate, Convert.ToDateTime(strCurrentDate))    // Less than or equal to
            );

            DateTime? latestCaptuteEndDate;
            FilterDefinition<Operator_History> filterByLatestdate = null;
            var latestCaptuteEndDateRecord = await _sqlDbContext.LatestCapturedEndDate.FirstOrDefaultAsync(e => e.collectionName == "Operator_History");
            if (latestCaptuteEndDateRecord != null)
            {
                ...
                if (latestCaptuteEndDate != DateTime.MinValue)
                {
                    string strlatestCaptuteEndDate = latestCaptuteEndDate?.ToString("yyyy-MM-ddTHH:mm:ss.fff+00:00");
                     filterByLatestdate = Builders<Operator_History>.Filter.And(
                        Builders<Operator_History>.Filter.Gt(x => x.enddate, Convert.ToDateTime(strlatestCaptuteEndDate)) // Greater than   
                        );

                }
            }

            FilterDefinition<Operator_History> combinedFilter = null;


            if (filterByLatestdate is not null)
            {
                combinedFilter = Builders<Operator_History>.Filter.And(
                filterByLatestdate,
                filter 
                );
            }
            ...
            var records = await _Operator_History.Find(combinedFilter)
                .Sort(Builders<Operator_History>.Sort.Descending(x => x.enddate)) // Sort by enddate descending
                //.Limit(25)
                .ToListAsync();


            var a = new List<Operator_History>();
            ...
            using (var transaction = await _sqlDbContext.Database.BeginTransactionAsync())
            {
                try
                {
                    if (records.Count > 0)
                    {
                        ...
                        {
                            var sqlRecord = new Sql_Operator_History
                            {
                            ...
                            await _sqlDbContext.Operator_History.AddAsync(sqlRecord);
                        }
                ...
                try {
                    if(records.Count > 0) {
                    var existingRecord = await _sqlDbContext.LatestCapturedEndDate.FirstOrDefaultAsync(e => e.collectionName == "Operator_History");
                    ...
                    else
                    {
                        ...
                        {
                            // Map the fields from MongoDB record to SQL Server record
                            collectionName = "Operator_History",
