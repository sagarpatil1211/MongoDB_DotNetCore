...
public partial class MessageHub : Hub
{
    ...
    private static IActorRef MessageHubMessageRetainer = null;
    ...
    {
        m_actorSystem ??= ActorSystem.Create("MessageHubActorSystem");
    }

    public static void InstantiateHub()
    {
        InstantiateActorSystem();
        MessageHubMessageRetainer = ActorSystem.ActorOf<MessageHubMessageRetainer>();
    }

    static MessageHub()
    {
        cnnRetainedMessages = DataAccessUtils.ProvisionAndCreateNewDBConnection("MessageHub");
        PerformDBMigrationForRetainedMessagesTable();
        PopulateRetainedMessagesInMemory();
    }

    private readonly IHubContext<MessageHub> m_ctx = null;

    public MessageHub(IHubContext<MessageHub> ctx)
    {
        m_ctx = ctx;
    }
    ...
    public override async Task OnDisconnectedAsync(Exception ex)
    {
        ...
        {
            //Log.Information($"Disconnected from Msg Hub : {Context.ConnectionId} at {DateTime.Now:dd/MM/yyyy HH:mm:ss.fff}");
    ...
    private async Task PostDecimalPacketToTopicInternal(string topic, DecimalPacket payload, bool retain)
    {
        if (retain)
        {
            var rmi = new RetainedMessageInfo(PayloadType_DecimalPacket, topic, payload.Serialize());

            m_retainedMessages.AddOrUpdate(topic, rmi, (t, msg) => rmi);
            MessageHubMessageRetainer?.Tell(rmi);
        }
    ...
    private async Task PostIntegerPacketToTopicInternal(string topic, IntegerPacket payload, bool retain)
    {
        if (retain)
        {
            var rmi = new RetainedMessageInfo(PayloadType_IntegerPacket, topic, payload.Serialize());
    ...
    private async Task PostLongPacketToTopicInternal(string topic, LongPacket payload, bool retain)
    {
        if (retain)
        {
            var rmi = new RetainedMessageInfo(PayloadType_LongPacket, topic, payload.Serialize());
    ...
    public async Task PostStringPacketToTopicInternal(string topic, StringPacket payload, bool retain)
    {
        if (retain)
        {
            var rmi = new RetainedMessageInfo(PayloadType_StringPacket, topic, payload.Serialize());
    ...
    private async Task PostPayloadPacketToTopicInternal(string topic, PayloadPacket payload, bool retain)
    {
        if (retain)
        {
            var rmi = new RetainedMessageInfo(PayloadType_PayloadPacket, topic, payload.Serialize());
    ...
    // ************************************************
    private static Tuple<string, StringPacket, bool> GetLastWillStringPacketMessage(string connectionId)
    {
    ...
    private static Tuple<string, IntegerPacket, bool> GetLastWillIntegerPacketMessage(string connectionId)
    {
    ...
    private static Tuple<string, LongPacket, bool> GetLastWillLongPacketMessage(string connectionId)
    {
    ...
    private static Tuple<string, DecimalPacket, bool> GetLastWillDecimalPacketMessage(string connectionId)
    {
    ...
    private static Tuple<string, PayloadPacket, bool> GetLastWillPayloadPacketMessage(string connectionId)
    {
